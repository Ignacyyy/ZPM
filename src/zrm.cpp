...
std::vector<std::string> listInstalledNativePackages(const AppContext& context) {
    ProcessResult result;
    if (context.packageManager == "apt") {
        result = capture({"dpkg-query", "-W", "-f=${binary:Package}\\t${Status}\\n"});
    ...
    for (const std::string& line : splitLines(result.output)) {
        if (context.packageManager == "apt") {
            ...
            const std::string status = trim(line.substr(separator + 1));
            if (status != "install ok installed") {
                continue;
            }

            addUniqueString(packages, line.substr(0, separator));
            continue;
        }
